...
config_dir = config.CONFIG_DIR.lstrip(".").lstrip("/").rstrip("/")
...
def controller(controller_name, methods):
    ...
    controller_path = f"{controllers_dir}/{snake_case_name}_controller.py"
...
def scaffold(name, fields):
    ...
    controller_path = f"{controllers_dir}/{plural_snake_case}_controller.py"
    os.makedirs(os.path.dirname(controller_path), exist_ok=True)
    with open(controller_path, "w") as f:
